...
#     def __init__(self, runNum = -1, eventNum = -1):
    ...
    def process(self, events):
        ...
        triJetSum = triJet.j1 + triJet.j2 + triJet.j3
        ...
        for syst in systList:
            ...
            for lepton in ['electron','muon']:
                ...
                #phosel = selection.all(*('jetSel','onePho',lepSel))
                #phosel = selection.all(*('jetSel','onePho',lepSel))
                #phosel = selection.all(*('onePho',lepSel))
                ...
                #print(evtWeight)
                #print(np.sum(selection.all(lepSel)))
                #print(selection.all('onePho'))
                #print(np.sum(phosel))
                #print(len(ak.flatten(tightPhoton[phosel])), len(phoCategory[phosel]), len(evtWeight[phosel]))
